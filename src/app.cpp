...
//    SoundController::getInstance().setup();
	...
	int h = max(
				...
				mapping_controller.getMapping()->getControl()->getProjector()->outputHeight().get());
	...
	control_window = gui.getRoot()->add<ControlWindow>();
...
void AppStart::update(){
		...
		paths_controller.update(mapping_controller.getMapping()->getControl()->getProjector(),
								traces_controller.getObjectController()->getDrawingObjects());
